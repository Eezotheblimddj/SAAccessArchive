...
class MainWindow(wx.Frame):
    ...
    def open_selected(self):
        ...
        if self.remote_mode:
            ...
            if item_type == "Remote Folder":
                ...
                self.status_label.SetLabel(
                    "Remote file selected: " + name
                    + "  (download not yet built)"
                )
                self.Layout()
        else:
    ...
    def on_up_level(self, event):
        if self.remote_mode:
            if self.remote_path in ("/", ""):
                self.status_label.SetLabel("Already at the remote top level.")
                self.Layout()
                return
    ...
    def on_connect(self, event):
        ...
        try:
            ...
            test_entries = self.remote_client.list("/")
